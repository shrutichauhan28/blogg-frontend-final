...
function displayArticle(article) {
    ...
    document.getElementById('post-image').src = article.imageLink || './images/recipes/11.jpg';
...
function displayComments(comments) {
    ...
    comments.forEach(comment => {
        ...
        // Create a Date object and format date and time
        const createdAt = new Date(comment.createdAt);
        const dateString = createdAt.toLocaleDateString(); // Date
        const timeString = createdAt.toLocaleTimeString(); // Time
        const dateTimeString = `${dateString} ${timeString}`; // Combined
        
        commentElement.innerHTML = `
            <div class="comment-meta">By ${comment.author.username} on ${dateTimeString}</div>
